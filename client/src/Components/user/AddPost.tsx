...
interface AddPostProps{
  ...
  setAddingPostLoading:React.Dispatch<React.SetStateAction<boolean>>
}

const AddPost:React.FC<AddPostProps>=({setNewPostAdded,handlePhotoSelect,image,setAddingPostLoading})=>{
        ...
        <div className=" w-full h-[10%] md:h-[15%] flex items-center space-x-2 md:space-x-5" >
          ...
          <div className=" flex bg-slate-50 w-[90%] h-full border-2 rounded-sm">
            <div className="w-[30%] md:w-[20%] h-full flex p-4 items-center justify-center ">
                ...
                <input   className=" w-14 h-8 md:w-48 md:h-20 absolute   opacity-0" accept="image/*" onChange={handlePhotoSelect} type="file"  />
            </div>
